-- Money Paws - Advanced Features Migration Script
-- Run this to add advanced breeding and tournament features to existing database
-- Developed and Designed by Ryan Coleman. <coleman.ryan@gmail.com>

-- Add species column to pets table if it doesn't exist
ALTER TABLE pets ADD COLUMN IF NOT EXISTS species VARCHAR(50) DEFAULT 'dog' AFTER original_name;
...
INSERT IGNORE INTO `pet_traits` (`trait_name`, `trait_category`, `trait_description`, `rarity`, `genetic_dominance`, `breeding_bonus`, `market_value_multiplier`) VALUES
-- Physical Traits
...
INSERT IGNORE INTO `breeding_compatibility` (`species_a`, `species_b`, `compatibility_score`, `success_rate_modifier`, `offspring_species`, `is_hybrid`) VALUES
...
CREATE INDEX IF NOT EXISTS idx_pet_traits_category ON pet_traits(trait_category);
CREATE INDEX IF NOT EXISTS idx_pet_traits_rarity ON pet_traits(rarity);
CREATE INDEX IF NOT EXISTS idx_pet_trait_assignments_pet_id ON pet_trait_assignments(pet_id);
CREATE INDEX IF NOT EXISTS idx_breeding_records_parents ON breeding_records(mother_id, father_id);
CREATE INDEX IF NOT EXISTS idx_tournaments_status ON tournaments(status);
CREATE INDEX IF NOT EXISTS idx_tournaments_game_mode ON tournaments(game_mode);
CREATE INDEX IF NOT EXISTS idx_tournament_participants_tournament_id ON tournament_participants(tournament_id);
CREATE INDEX IF NOT EXISTS idx_tournament_matches_tournament_round ON tournament_matches(tournament_id, round_number);
CREATE INDEX IF NOT EXISTS idx_pet_battle_stats_elo ON pet_battle_stats(elo_rating);
CREATE INDEX IF NOT EXISTS idx_tournament_events_tournament_id ON tournament_events(tournament_id);

-- Update existing pets to have random species if NULL
UPDATE pets SET species = 'dog' WHERE species IS NULL OR species = '';

-- Initialize battle stats for existing pets
INSERT IGNORE INTO pet_battle_stats (pet_id)
SELECT id FROM pets WHERE life_status = 'alive';

-- Migration complete message
SELECT 'Advanced Features Migration Complete!' as status;
